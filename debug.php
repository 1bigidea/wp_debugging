    ...
    class trrDebugClass
    {
        ...
        function __construct()
        {
            if (defined('TRR_DEBUG') && TRR_DEBUG) {
                ...
                add_action('doing_it_wrong_run', array($this, 'doing_it_wrong_backtrace'), 10, 3);
            }
        ...
        function backTrace()
        {
            kickout('backtrace', 'Backtrace from ' . $_SERVER['HTTP_HOST'], $this->get_backTrace());
        }

        function doing_it_wrong_backtrace($function, $message, $version)
        {
            kickout('doing_it_wrong_' . $function, wp_debug_backtrace_summary(), FILE_APPEND);
        }
    }
}
if ($debug)
...
if (!function_exists('kickout')) {
    function kickout()
    {
        ...
        file_put_contents(WP_CONTENT_DIR . '/_' . $file_output . '.txt', $output, ($file_append ? FILE_APPEND : 0));
    }
}
if (!function_exists('kickout_html')) {
    function kickout_html()
    {
        ...
        file_put_contents(WP_CONTENT_DIR . '/_' . $file_output . '.html', $output, ($file_append ? FILE_APPEND : 0));
    }
}
if (!function_exists('whereCalled')) {
    function whereCalled($level = 1)
    {
        $trace = debug_backtrace($level);
